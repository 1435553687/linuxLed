...
void MainWindow::Textt()//读取串口数据并显示出来
{
    ...
    if(str!=NULL)
    {
        if(str ==QString::fromLocal8Bit("/1"))
        {
        ...
        else if(str ==QString::fromLocal8Bit("/3"))
        {
            pthread_create(&p1, NULL,thrd_yqs, NULL);
            ui->label->setText("3");
            n=3;
        }
       else if(str ==QString::fromLocal8Bit("/4"))
        {
             ti=3;
       }
        else if(str ==QString::fromLocal8Bit("/5"))
       {
            ti=2;
        }
        else if(str ==QString::fromLocal8Bit("/6"))
        {
            ti=1;
        }
    }
    ...
    ui(new Ui::MainWindow)
{
    ...
    if (f.exists())
        {
                fb = open("/dev/led", QIODevice::ReadWrite);
                if (fb < 0)
                {
...
readTimer=new QTimer(this);
    readTimer->start(100);
    myCom = new Posix_QextSerialPort("/mnt/ttyS1",QextSerialBase::Polling);
